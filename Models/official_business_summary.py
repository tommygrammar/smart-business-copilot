...
def create_business_summary():
    ...
    strong_drivers = re.findall(r"•\s*([\w_]+):", strengths_text)
    ...
    narrative = (
        ...
        "**Key Revenue Drivers:**\n\n"
        f"  **Strongest:** {', '.join(strong_drivers) if strong_drivers else 'N/A'}\n\n"
        f"  **Weakest:** {', '.join(weak_drivers) if weak_drivers else 'N/A'}\n\n"
    )
